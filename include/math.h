...
template <class FLOAT_TYPE, size_t N> struct Vector {
    ...
    // creates a unit vector pointing to the given angle (in radians) in the x/y
    // plane angle = 0 points in the direction of the x-axis
    ...
    template <class F, size_t K> friend Vector<F, K> operator*(F scalar, Vector<F, K> value);
