...
template <class FLOAT, size_t N> struct Ray {
    Vector<FLOAT, N> origin, direction;
};

// collection of intersection specific values, like intersection point, normal etc
template <class FLOAT, size_t N> struct Intersection_Context {
...
/*
 a axis aligned bounding box (aabb) with a center point
  ...
  +----------------------+
*/
template <class FLOAT, size_t N> class AxisAlignedBoundingBox {
  protected:
    Vector<FLOAT, N> center, half_edge_length;
  ...
  public:
    ...
    Vector<FLOAT, N> sweep_intersects(AxisAlignedBoundingBox<FLOAT, N> aabb,
                                      Vector<FLOAT, N>                 direction) const;
};

// a sphere with a center and a radius
template <class FLOAT, size_t N> class Sphere {
  protected:
...
template <class FLOAT, size_t N> class Triangle {
  ...
  public:
    ...
    Triangle(Vector<FLOAT, N> a, Vector<FLOAT, N> b, Vector<FLOAT, N> c, Vector<FLOAT, N> na,
    ...
    bool intersects(const Ray<FLOAT, N>& ray, Vector<FLOAT, N>& normal,
                    Vector<FLOAT, N>& intersection, FLOAT& u, FLOAT& v, FLOAT& t) const;
