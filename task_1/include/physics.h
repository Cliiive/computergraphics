...
template <class FLOAT_TYPE, size_t N> class BoundingVolumeCircle : public Sphere<FLOAT_TYPE, N> {
  public:
...
template <class FLOAT_TYPE, size_t N> class BoundingVolumeHyperRectangle {
  protected:
    Vector<FLOAT_TYPE, N> position;
    Vector<FLOAT_TYPE, N> edge_lengths;

  public:
    BoundingVolumeHyperRectangle(Vector<FLOAT_TYPE, N> position,
                                 Vector<FLOAT_TYPE, N> edge_lengths);
...
template <class FLOAT_TYPE, size_t N, class BV> class Body {
  protected:
    BV                    bounding;
    ...
    std::function<void(Body<FLOAT_TYPE, N, BV>*, FLOAT_TYPE)>
        fix;  // fix object values after movement
  ...
  public:
    Body(
        BV bounding_volume, Vector<FLOAT_TYPE, N> velocity, FLOAT_TYPE max_velocity = 1.0,
        FLOAT_TYPE min_velocity = 0.0, FLOAT_TYPE angle = 0.0,
