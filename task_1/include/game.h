...
enum class GameEvent : short {
    ...
    ship_destroyed,
    ship_thrust,
    small_saucer_destroyed,
    big_saucer_destroyed,
    end_of_level,
    next_level_started,
    new_ship_spawned,
    torpedo_fired
...
class Torpedo : public TypedBody {
  ...
  public:
    Torpedo() : Torpedo(Vector2df{0.0f, 0.0f}, 0.0f, Vector2df{1.0f, 1.0f}) {}
    ...
    Torpedo(Vector2df position, float angle, Vector2df velocity)
        : TypedBody(BodyType::torpedo,
                    Body2df{BoundingVolume2df{position + 14.0f * Vector2df(angle), 1.0},
                            velocity + 1.1f * MAX_SPEED / 2.0f * Vector2df(angle), MAX_SPEED, 0.0f,
  ...
  public:
    ...
    Spaceship(Vector2df position)
        : TypedBody(BodyType::spaceship,
                    Body2df{BoundingVolume2df{position, 10.0f}, Vector2df{0.0f, 0.0f}, MAX_SPEED,
                            0.0f, 0.0f, spaceship_fix}) {
  ...
  public:
    ...
    SpaceshipDebris(Vector2df position = Vector2df{0.0, 0.0}, float angle = 0.0)
        : TypedBody(BodyType::spaceship_debris,
                    Body2df{BoundingVolume2df{position, 0.0}, Vector2df{0.0, 0.0}, 384.0, 0.0,
                            angle, displacement_fix}) {
...
class Saucer : public TypedBody {
    ...
    char                   precise_shoot_counter =
        0;  // every sixth torpedo of a small saucer shoots in direction to the spaceship
  public:
...
class Game {
    ...
    Saucer          saucer{1, Vector2df{70.0, 70.0},
                  [&](Body2df* body, float time) -> void { this->saucer_fix(body, time); }};
    SpaceshipDebris debris;
    std::array<Asteroid, MAXIMUM_ASTEROIDS_SPAWNING * 4>
                           asteroids;  // maximal possible asteroids, in original it is 26
    std::array<Debris, 10> debrises;
    std::vector<GameEvent> game_events;
    short                  no_of_ships = NO_OF_SHIPS_AT_START;
    size_t                 current_no_of_asteroids =
        NO_OF_ASTEROIDS_AT_START;  // no of asteroids at start of current level
