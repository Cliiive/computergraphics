...
Asteroid::Asteroid(short size)
    : TypedBody(BodyType::asteroid, Body2df{BoundingVolume2df{Vector2df{128.0f + 768.0f * dis(gen),
                                                                        64.0f + 640.0f * dis(gen)},
                                                              size * 11.0f},
                                            Vector2df{0.5f - dis(gen), 0.5f - dis(gen)}, 348.0, 0.0,
                                            0.0, displacement_fix}),
      size(size), rock_type(std::trunc(4 * dis(gen))) {
    ...
    if (shoot_cooldown.get_time() <= 0.0 && !is_marked_for_deletion()) {
        ...
        if (i < torpedos.size()) {
            if (size == 0 && precise_shoot_counter <= 0 && !game.ship.is_marked_for_deletion()) {
                ...
                torpedos[i] =
                    Torpedo{get_position(), direct_shot.angle(0.0f, 1.0f), get_velocity()};
...
void Game::destroy_asteroid(Asteroid* asteroid) {
    ...
    *debris        = Debris(asteroid->get_position());
    ...
    switch (asteroid->get_size()) {
        case 1:
            game_events.push_back(GameEvent::small_asteroid_destroyed);
            break;
        case 2:
            game_events.push_back(GameEvent::medium_asteroid_destroyed);
            break;
        case 3:
            game_events.push_back(GameEvent::large_asteroid_destroyed);
            break;
    }
...
void Game::torpedo_hits_asteroid(Torpedo* torpedo, Asteroid* asteroid) {
    ...
    if (ship.contains_torpedo(torpedo)) {
        switch (asteroid->get_size()) {
            case 1:
                add_score(POINTS_SMALL_ASTEROID);
                break;
            case 2:
                add_score(POINTS_MEDIUM_ASTEROID);
                break;
            case 3:
                add_score(POINTS_LARGE_ASTEROID);
                break;
        }
    }
}
...
void Game::destroy_saucer(Saucer* saucer) {
    switch (saucer->get_size()) {
        case 0:
            game_events.push_back(GameEvent::small_saucer_destroyed);
            break;
        case 1:
            game_events.push_back(GameEvent::big_saucer_destroyed);
            break;
    }
...
bool Game::area_free_of_asteroids(BoundingVolume2df* bounding) {
    return physics.is_area_free_of_bodies(bounding, [bounding](Body2df* body) -> bool {
...
void Game::new_saucer() {
    if (saucer.is_marked_for_deletion()) {
        ...
        if (area_free_of_asteroids(&body)) {
            ...
            saucer = Saucer{type, position, [&](Body2df* body, float time) -> void {
                                this->saucer_fix(body, time);
                            }};
...
bool Game::check_collision(Body2df* body1, Body2df* body2) {
    ...
    bool saucer =
        typed_body1->get_type() == BodyType::saucer || typed_body2->get_type() == BodyType::saucer;
    return (torpedo && (asteroid || spaceship || saucer)) || (asteroid && (saucer || spaceship)) ||
           (saucer && spaceship);
}
