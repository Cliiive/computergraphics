...
AxisAlignedBoundingBox<FLOAT, N>::AxisAlignedBoundingBox(Vector<FLOAT, N> center,
                                                         Vector<FLOAT, N> half_edge_length)
...
bool AxisAlignedBoundingBox<FLOAT, N>::intersects(AxisAlignedBoundingBox<FLOAT, N> aabb) const {
    ...
    for (size_t i = 0; i < N; i++) {
        intersects &=
            (center[i] - half_edge_length[i] - aabb.half_edge_length[i] <= aabb.center[i]) &
...
bool AxisAlignedBoundingBox<FLOAT, N>::intersects(AxisAlignedBoundingBox<FLOAT, N> aabb,
    ...
    AxisAlignedBoundingBox<FLOAT, N> center_extended =
        AxisAlignedBoundingBox(center, half_edge_length + aabb.half_edge_length);
...
template <class FLOAT, size_t N>
Vector<FLOAT, N>
AxisAlignedBoundingBox<FLOAT, N>::sweep_intersects(AxisAlignedBoundingBox<FLOAT, N> aabb,
                                                   Vector<FLOAT, N> direction) const {
    ...
    for (size_t i = 0; i < N; i++) {
        tmin[i] = (center[i] - aabb.center[i] - (half_edge_length[i] + aabb.half_edge_length[i])) /
                  direction[i];
        tmax[i] = (center[i] - aabb.center[i] + (half_edge_length[i] + aabb.half_edge_length[i])) /
                  direction[i];
    }
    FLOAT tminimum = -INFINITY;
    FLOAT tmaximum = INFINITY;
...
template <class FLOAT, size_t N>
bool Triangle<FLOAT, N>::intersects(const Ray<FLOAT, N>&            ray,
...
template <class FLOAT, size_t N>
bool Triangle<FLOAT, N>::intersects(const Ray<FLOAT, N>& ray, Vector<FLOAT, N>& normal,
                                    Vector<FLOAT, N>& p, FLOAT& u, FLOAT& v, FLOAT& t) const {
...
bool refract(FLOAT refraction_index, Vector<FLOAT, N> normal, Vector<FLOAT, N> direction,
    ...
    FLOAT sin_phi_squared =
        refraction_index * refraction_index * (static_cast<FLOAT>(1.0) - cos_theta * cos_theta);
